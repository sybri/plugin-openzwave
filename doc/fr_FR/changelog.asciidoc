...
** Ne pas tester si un nodeId existe lors de la suppresion d'un association.
** Restoration de la notification de consigne en attente sur les thermostats.
** Envoie de la Scene Activation en instance 1. 
** Ne plus afficher le cadenas dans l'écran de santée sur les modules ne supportant pas la Classe de Commande Securitée.
** Répétition de valeur sur les télécommande avant la fin de l'interview (kyefob, minimote).
** Modifier un paramètre de type liste par valeur via une commande Action.
** Modifier un un paramètre sur un module sans configuration définie.
...
*** Modification de l'ensemble des routes existantes, les scripts devront être adapté si utilisation de l'API ZWave.
*** Renforcement de la sécurité, seul des appels sont écouté sur le serveur REST.
*** Utilisation de la clé API ZWave est requise pour lancer des requêtes REST.
...
** Optimisations du code de la librairie openzwave.
*** Fibaro FGK101 n'a plus à compléter l'interview pour annoncer un changement d'état.
*** La commande bouton relâcher (Stop d'un volet) ne force plus la mise à jour de l’ensemble des valeurs du module (diminution de la queue de message).
